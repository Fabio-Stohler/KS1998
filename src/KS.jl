...
# Solve for the aggregate law of motion
function solve_ALM_BK(plotting = false, plotting_check = false)
    # generate structures
    mpar = ModelParameters()
    npar = NumericalParameters()
    ...
    @set! npar.mesh_k = repeat(reshape(k, (1, ngridk, 1, 1, 1)), outer = [npar.ngridb, 1, ngridkm, nstates_ag, nstates_id]) # Does this work when changing the type of the structure?
    ...
    inc::Array = [
        w_t .* npar.mesh_ϵ .* mpar.l_bar .+ mpar.μ .* (w_t .* (1 .- npar.mesh_ϵ)) .-
        mpar.μ .* (w_t .* (1 .- L) ./ L) .* npar.mesh_ϵ, # labor income
        npar.mesh_b, # bond income
        r_t .* npar.mesh_k, # capital income
    ]
    ...
    while dif_B > npar.ϵ_B && iteration < npar.iter_max
        # Solve for HH policy functions at a given law of motion
        c, k1, b1 = solve_HH(A, B, inc, r_t, npar, mpar)
        ...
        # Generate time series and cross section of capital
        km_ts, q_ts, distr1 = aggregate_st(distr, k1, b1, npar.ag_shock, npar)
        """
        run regression: log(km') = B[j,1]+B[j,2]log(km) for aggregate state
        """
        x = log.(km_ts[npar.burn_in:end-1])[:]
