...
@doc raw"""
    solve_HH(M::Chain, inc::Vector, r_t::Array, npar::NumericalParameters, mpar::ModelParameters, verbose::Bool = false)

Solving the household problem via endogeneous grid methods using a neural network approximation of the perceived law of motion.

Inputs:
- M::Chain:                          Neural network model for the perceived law of motion
- inc::Vector{Array{Float64,4}}:     State dependent income of households
- r_t::Array{Float64,4}:             State dependent interest rate
- npar::NumericalParameters:         Numerical parameters of the model
- mpar::ModelParameters:             Economic parameters of the model
- verbose::Bool:                     Print progress to screen
...
function solve_HH(
    ...
    while dist > npar.ϵ_k && count < npar.iter_max_k # Iterate consumption policies until converegence
        count = count + 1
        # Take expectations for labor income change
        update_EVk!(EVk, Vk, rmu, M, mpar, npar)
